...
class StripeService
{
    ...
    {
        $this->secret = systemflag('paymentModeEnvStripe') ? systemflag('stripeLiveSecretkey') : systemflag('stripeTestingSecretkey');
        $this->key    = systemflag('paymentModeEnvStripe') ? systemflag('stripeLivePublishkey') : systemflag('stripeTestingPublishkey');
    ...
    public function createIntent($order, $user)
    {
        try {
            if($order->total_amount < 1){
                $totalAmountForGateway = round($order->total_amount * 100);
            }else{
                $totalAmountForGateway = $order->total_amount * 100;
            }
            $intent = PaymentIntent::create([
                'amount' => $totalAmountForGateway, // cents
